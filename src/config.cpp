#include "config.h"

const std::string config::URL_REVOLVAIR = "https://staging.revolvair.org/api/revolvair/stations/cegep-de-ste-foy/";
const std::string config::URL_EPA = "https://staging.revolvair.org/api/revolvair/aqi/usepa/fr";
const char* config::WIFI_NAME = "BELL201";
const char* config::WIFI_PASSWORD = "alex123456";
const char* config::API_POST_URL = "https://staging.revolvair.org/api/revolvair/stations/dual/measures";
